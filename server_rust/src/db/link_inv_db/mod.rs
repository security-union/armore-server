use super::{telemetry_db::postgres_to_api, transaction};
use crate::model::{
    APIInternalError, AcceptedNotificationData, InvitationState, LinkActionData, LinkCreationData,
    PostgresConnection,
};
use crate::publish_websocket_messages::{get_rabbitmq_uri, send_broadcast_notification};
...
fn push_accepted_notification(
    ...
    RabbitConnection::insecure_open(&get_rabbitmq_uri())
        .and_then(|mut rabbit_conn: RabbitConnection| {
...
pub fn assert_not_friends(
    ...
    conn.query(
        "SELECT * FROM users_followers_state WHERE username IN ($1, $2) AND username_follower IN ($1, $2)",
        &[user1, user2],
    )
    .map_err(|w| postgres_to_api(w))
    .and_then(|rows| {
